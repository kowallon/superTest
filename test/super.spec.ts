...
//describe('Happy path API Tests', () => {
    ...
    //     //zainstaluj npm install chai-xml --save-dev
    ...
    //     expect(response.status).to.equal(200);
    //     //expect(response).to.be.a.jsonFile()
    // });
    ...
    //     //zainstaluj npm install chai-sorted
    //     const response = await request(`${baseUrl}${apiKey}`)
    //     .get(`${jsonType}&order_by=name&sort_order=desc`)

    //     let names: Object[] = []

    // expect(response.body.releases).to.be.sortedBy("name", {descending: true})
    //     expect(response.status).to.equal(200)

    //     for(let release of response.body.releases){
    //         names.push(release.name)
    //     }
    //     //jak zadziała to na dole to można wywalić pętle
    //     expect(response.body.releases).to.be.sorted({descending: true})
    // })
    ...
    //     for(let release of response.body.releases){
    //         expect(release.realtime_start).to.be.afterOrEqualDate(startDate)
    //         expect(release.realtime_end).to.be.beforeOrEqualDate(endDate)
    //     }
    // })
//});
